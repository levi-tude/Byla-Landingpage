...
| **workflow-supabase-webhook-google-sheets-export.json** | **INSERT** em `transacoes` (webhook Supabase) → backend `POST /api/planilha-entrada-saida/montar-linhas` → Google Sheets **Movimentações**. URL padrão = **Render** (`byla-backend.onrender.com`). No **host n8n**: obrigatório **BYLA_SYNC_SECRET**; opcional **BYLA_BACKEND_URL**. [HOST_ENV_BYLA.md](./HOST_ENV_BYLA.md). SQL: `scripts/supabase-mapeamento-categoria-e-view-export.sql`. Guia: [docs/N8N_WEBHOOK_EXPORT_PLANILHA.md](../docs/N8N_WEBHOOK_EXPORT_PLANILHA.md). |
| **workflow-supabase-bulk-export-google-sheets-once.json** | Carga **única** (manual): mesma regra HTTP + Sheets. Mesmas variáveis no host. |
